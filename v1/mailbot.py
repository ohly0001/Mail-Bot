...
try:
    while True:
        ...
        inbox = mail.fetch_unread() #TODO get references and recipients (cc & to)
        ...
        if inbox_size > 0:
            ...
            for i, in_msg in enumerate(inbox, start=1):
                ...
                # TODO command parsing (with handling for batched commands (1 per line in a message))
                #commands (!... client level, /... thread level)
                for line in in_msg['body_text'].strip().split('\n'):
                    if line.startswith('!'):
                        ...
                        if cmd == 'mode natural': 
                            ...
                            # Don't respond but persist thread to db as received
                            # Ignore commands into switched from off to other mode
                            pass
                        elif cmd.startswith('include '): #`me` is alias for self, `cc` is for all cc people, comma seperated, `all`` removes filtering
